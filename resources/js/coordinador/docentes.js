/** Lista de docentes con carga horaria real desde /api/coordinador/docentes. */
...
export function initCoordinadorDocentes() {
    const tbody = document.getElementById('coord-docentes-tbody');
    if (!tbody) return;

    fetch('/api/coordinador/docentes', { headers: { Accept: 'application/json' } })
        .then((res) => res.json())
        .then((data) => {
            ...
            tbody.innerHTML = docentes.length
                ...
                : '<tr><td colspan="5" class="coord-portafolio-empty">No hay docentes registrados.</td></tr>';

            const root = document.getElementById('coord-docentes-kpis');
        ...
        .catch((error) => {
            tbody.innerHTML = '<tr><td colspan="5" class="coord-portafolio-empty">No se pudo cargar la lista de docentes.</td></tr>';
            console.error(error);
        });
}

document.addEventListener('DOMContentLoaded', initCoordinadorDocentes);
