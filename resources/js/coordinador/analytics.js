...
function renderRendimiento(cursos) {
    const canvas = document.getElementById('coord-chart-rendimiento');
    if (!canvas) return;
    ...
    new Chart(canvas, {
        ...
        data: {
            ...
            datasets: [{
                label: 'Rendimiento (%)',
                data: cursos.map((c) => c.porcentaje ?? 0),
...
function renderPortafolio(portafolio) {
    const canvas = document.getElementById('coord-chart-portafolio');
    if (!canvas) return;
...
function renderSilabo(porCiclo) {
    const canvas = document.getElementById('coord-chart-silabo');
    if (!canvas) return;
...
function renderRiesgo(datos) {
    const canvas = document.getElementById('coord-chart-riesgo');
    if (!canvas) return;

    new Chart(canvas, {
        type: 'line',
        data: {
            labels: datos.map((d) => `Ciclo ${d.ciclo}`),
            datasets: [
                {
...
export function initCoordinadorAnalytics() {
    const root = document.getElementById('coord-chart-rendimiento');
    if (!root) return;

    fetch('/api/coordinador/analitica', { headers: { Accept: 'application/json' } })
        .then((res) => res.json())
        .then((data) => {
            renderRendimiento(data.rendimiento_cursos ?? []);
...
document.addEventListener('DOMContentLoaded', initCoordinadorAnalytics);
